...
public interface AccountRepository extends CrudRepository<Account, Integer> {
    ...
    List<Account> getUserAccountsById(@Param("user_id") String user_id);

    @Query(value="SELECT sum(balance) FROM account WHERE user_id = :user_id",nativeQuery= true)
    BigDecimal getTotalBalance(@Param("user_id") String user_id);

    @Query(value="SELECT balance FROM account WHERE user_id = :user_id AND account_id = :account_id",nativeQuery= true)
    Double getAccountBalance(@Param("user_id") String user_id, @Param("account_id") int account_id);
    ...
    @Transactional
    void createBankAccount(@Param("user_id") String user_id,
